...
export async function onRequestGet({ request }) {
  ...
  // Predicted scoreline removed (Aug 2026): an exact final score is not
  // something we can call, and the "2ND"/"ALT" boxes plus the goals-line and
  // likely-total figures beside them were derived from it, not predicted.
  // The card now carries the pick, the confidence and the risk band.
  const scoreBoxes = '';
  const goalsLineText = riskWord;
